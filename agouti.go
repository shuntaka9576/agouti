...
// New pages will accept invalid SSL certificates by default. This
// may be disabled using the RejectInvalidSSL Option.
func ChromeDriver(binaryPath string, options ...Option) *WebDriver {
	var binaryName string
	if runtime.GOOS == "windows" {
		binaryName = filepath.Join(binaryPath, "chromedriver.exe")
	} else {
		binaryName = filepath.Join(binaryPath, "chromedriver")
	}
